...
public class F {
	...
	{
		addRangeFenwick(ft0, ft1, l, r-1, x);
	}

	long sum(int l, int r)
	{
		return sumRangeFenwick(ft0, ft1, r-1) - sumRangeFenwick(ft0, ft1, l-1);
	}
	...
	int kth(int K)
	{
		...
		while(high - low > 1){
			int h = high+low>>1;
			if(sumRangeFenwick(ft0, ft1, h-1) < K){
				low = h;
			}else{
				high = h;
			}
...
//			tr(restoreRangeFenwick(ft0, ft1));
		}
