...
class Solution {
    public int largestSubmatrix(int[][] matrix) {
        ...
        for(int i=0;i<matrix.length;i++)
        {
            ...
            int size=0,curr=0;
            /*
                In current row, total entries with value k or greater than k will contribute the squares of size=k*(number of squares of size k or greater than k).Largest square ending at current row= k * (all entries greater than or equal to k in current row) for all ks in current row
            */
            while(!pq.isEmpty())  
            {
                curr=pq.peek(); 
